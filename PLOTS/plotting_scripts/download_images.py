...
for comp in bayes_comp:
	...
	for i in xrange(len(present_cats)):
		if 'gleam' in present_cats[i]:
			present_cats[i] = 'gleam'
	
	if src_all.names[0] in check_names:
	
		##Currently only want images from vlssr, sumss and nvss 
		image_cats = [cat for cat in ['vlssr','sumss','nvss','gleam'] if cat in present_cats]
		
		##Test to see if images have already been downloaded
		#image_locs = "/home/jline/Documents/cataloguing/FHD/carroll/paper/complete_v11/extended_fits"
		image_locs = "./extended_fits"
		image_files =[1 for cat in ['vlssr','sumss','nvss','gleam'] if os.path.exists("%s/%s_%s.fits" %(image_locs,src_all.names[0],cat))==True]
		
		if len(image_files) < len(image_cats):
			...
			if len(image_cats)==1:
				mv_cmd = 'mv %s.fits %s/%s_%s.fits' %(src_all.names[0],image_locs,src_all.names[0],image_cats[0])
				subprocess.call(mv_cmd,shell=True)
			else:
				for i in xrange(len(image_cats)):
					#print 'mv %s_%d.fits %s_%s.fits' %(src_all.names[0],i+1,src_all.names[0],image_cats[i])
					mv_cmd = 'mv %s_%d.fits %s/%s_%s.fits' %(src_all.names[0],i+1,image_locs,src_all.names[0],image_cats[i])
					subprocess.call(mv_cmd,shell=True)
					
			##You can troll the GLEAM server and just fiddle the ra,dec position in one sky_command
			
			gleam_command = 'wget "http://store04.icrar.org:7777/GLEAMCUTOUT?radec=%.10f,%.10f&radius=0.15&file_id=mosaic_Week2_170-231MHz.fits&regrid=1&projection=SIN&fits_format=1" -O %s/%s_gleam.fits' %(src_all.ras[0],src_all.decs[0],image_locs,src_all.names[0])
